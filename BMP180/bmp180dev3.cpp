...
double bmp_sealevel(double p,double StationAlt){
  //return p / pow((1-(StationAlt/145437.86)),5.255)*100 ; //return Pa based on feet
  return (p / pow((1-(StationAlt/44330)),5.255))*100 ; //return Pa based on meter
}
...
int main(int argc, char **argv)
{
   ...
   fd=i2c_Open(std::string(I2CBus));
	...
	sprintf (buffer, "Pressure\t:%.2f Pa\tThis is known as station pressure\""
                ,pressure*100);
	str = command + buffer;
	system(str.c_str());

	sprintf (buffer, "Altitute\t:%.2f m\tThis is known as a Pressure Altitude\""
                ,bmp_altitude(pressure));
	str = command + buffer;
	system(str.c_str());

	sprintf (buffer, "Sea Level\t:%.2f Pa\""
                ,bmp_sealevel(pressure,10)); //10 meters is my station elevation
	str = command + buffer;
	system(str.c_str());
